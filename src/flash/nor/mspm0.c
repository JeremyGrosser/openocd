...
static int mspm0_check_device_info(uint32_t deviceid, uint32_t userid)
{
	/* check ALWAYS_1 and START bits are set before anything else */
	if (!(deviceid & BIT(0)) || !(userid & BIT(31)))
		return ERROR_FAIL;

	uint32_t familyid = deviceid & 0x0FFFFFFE;
	const char *family_str = NULL;

	/* family definitions extracted from CCS .gel files */
	if (familyid == 0x0BB8202E)
		family_str = "MSPM0L11xx/MSPM0L13xx";
	if (familyid == 0x0BB8802E)
		family_str = "MSPM0G1x0x/MSPM0G3x0x";
	if (familyid == 0x0BB9F02E)
		family_str = "MSPM0L122x/MSPM0L222x";
	if (familyid == 0x0BBA102E)
		family_str = "MSPM0C110x";

	if (!family_str) {
		LOG_ERROR("Unknown MSPM0 family 0x%08" PRIx32 "", familyid);
		return ERROR_FAIL;
	}

	uint32_t version = (deviceid >> 28);
	uint32_t majorrev = (userid >> 28) & 0x7;
	uint32_t minorrev = (userid >> 24) & 0xF;
	uint32_t variant = (userid >> 16) & 0xFF;
	uint32_t part = userid & 0xFFFF;
...
{
	struct target *target = bank->target;
	uint32_t sramflash, deviceid, userid;
	int err;

	err = target_read_u32(target, 0x41C40004, &deviceid);
	if (err != ERROR_OK)
		return err;

	err = target_read_u32(target, 0x41C40008, &userid);
	if (err != ERROR_OK)
		return err;

	LOG_DEBUG("mspm0 read factory DEVICEID = 0x%08" PRIx32 " USERID = 0x%08" PRIx32 "", deviceid, userid);

	err = mspm0_check_device_info(deviceid, userid);
	if (err != ERROR_OK)
		return err;
	...
	err = target_read_u32(target, 0x41C40018, &sramflash);
	if (err != ERROR_OK)
		return err;
